...
使用方式：
  python twse_daily_fetch.py                   # 抓今日資料（上市＋上櫃）
  python twse_daily_fetch.py 20240101          # 抓指定日期
  python twse_daily_fetch.py --merge 6488      # 合併環球晶全部歷史 CSV
  python twse_daily_fetch.py --ta   6488       # 計算技術指標
  python twse_daily_fetch.py --schedule        # 啟動每日自動排程
  python twse_daily_fetch.py --range 20250101 20250530  # 批次抓取日期區間
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="台股每日資料抓取程式",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用範例：
  python twse_daily_fetch.py                              抓今日資料（上市＋上櫃）
  python twse_daily_fetch.py 20250530                     抓指定日期
  python twse_daily_fetch.py --range 20250101 20250530    批次抓日期區間
  python twse_daily_fetch.py --merge 2330                 合併台積電歷史 CSV
  python twse_daily_fetch.py --merge 6488                 合併環球晶（上櫃）歷史 CSV
  python twse_daily_fetch.py --ta    2330                 計算台積電技術指標
  python twse_daily_fetch.py --merge-all twse_institutional  合併上市法人資料
  python twse_daily_fetch.py --merge-all tpex_institutional  合併上櫃法人資料
  python twse_daily_fetch.py --schedule                   啟動每日自動排程
        """
    )
    parser.add_argument("date", nargs="?", help="指定日期 YYYYMMDD")
    parser.add_argument("--range", nargs=2, metavar=("START", "END"),
                        help="批次抓取日期區間 YYYYMMDD YYYYMMDD")
    parser.add_argument("--merge", metavar="STOCK_NO",
                        help="合併指定股票代號的歷史 CSV")
    parser.add_argument("--merge-all", metavar="TYPE",
                        choices=[
                            "twse_stock", "twse_index",
                            "twse_institutional", "twse_margin",
                            "tpex_stock", "tpex_index",
                            "tpex_institutional", "tpex_margin",
                        ],
                        help="合併所有日期的同類型資料（twse_*/tpex_*）")
    parser.add_argument("--ta", metavar="STOCK_NO",
                        help="計算指定股票的技術指標（MA/RSI/MACD/KD）")
    parser.add_argument("--schedule", action="store_true",
                        help="啟動每日自動排程（需安裝 schedule 套件）")
...
def main():
    ...
    if args.schedule:
        ...
        merge_stock_data(args.merge)

    elif args.merge_all:
        merge_all_days(args.merge_all)

    elif args.ta:
        compute_ta_for_stock(args.ta)
        ...
        try:
            ...
            logger.error("日期格式錯誤：%s，請使用 YYYYMMDD", date_str)
            sys.exit(1)
        run_daily(date_str)
